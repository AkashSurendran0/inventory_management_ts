...
export const getAllCustomers = async (query: string | null, page: number, limit: number): Promise<{result: Response}> => {
    return await api.get(`${backendRoute}/v1/customer/customers?query=${query}&page=${page}&limit=${limit}`)
}
